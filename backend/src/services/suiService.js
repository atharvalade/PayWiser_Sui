...
class SuiService {
  ...
  async getBalance(address) {
    try {
      ...
      // Get USDC balance
      const usdcBalance = await this.client.getBalance({
        owner: address,
        coinType: this.usdcCoinType
      });

      // Debug logging
      console.log('🔍 Debug - USDC Balance Query:');
      console.log('   Address:', address);
      console.log('   USDC Coin Type:', this.usdcCoinType);
      console.log('   Raw USDC Response:', JSON.stringify(usdcBalance, null, 2));
